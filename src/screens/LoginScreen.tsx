...
import { normalizePlate, validPlate, plateEmail } from '../../functions/validation';
...
export function LoginScreen() {
  ...
  async function submit() {
    setError('');
    if (!(administrative ? validEmail(email) : validPlate(email)) || !password) { setError(administrative ? 'Informe um e-mail válido e sua senha.' : 'Informe uma placa válida e sua senha.'); return; }
    setBusy(true);
    try { await login(administrative ? email : plateEmail(email), password); } catch (e) { setError(errorMessage(e)); } finally { setBusy(false); }
  }
  return <Screen style={{ maxWidth: 520, paddingTop: 30 }}>
    ...
    <View style={{ gap: 8 }}><Text accessibilityRole="header" style={shared.title}>{administrative ? 'Acesso administrativo' : 'Acompanhe seu veículo'}</Text><Text style={shared.muted}>{administrative ? 'Entre com a conta autorizada da oficina.' : 'No primeiro acesso, use a placa e a senha inicial informada pela oficina.'}</Text></View>
    <Notice text={error} />
    ...
    <Field label="Senha" placeholder="Sua senha" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} autoCapitalize="none" autoComplete="current-password" editable={!busy} onSubmitEditing={submit} />
