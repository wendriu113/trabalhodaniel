...
import { clientStage, dateLabel, money } from '../utils/domain';
  ...
  return <Screen><View style={shared.between}><Icon name="car-sport-outline" size={40} /><Button title="Sair da conta" variant="secondary" onPress={() => { logout().catch(e => setError(errorMessage(e))); }} /></View>
    ...
    {pending > 0 ? <Loading /> : !error && <>
      ...
      {[...orders].sort((a, b) => b.createdAt.toMillis() - a.createdAt.toMillis()).map(order => {
        const open = openOrderId === order.id, visibleStatus = clientStage(order.status);
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={`${open ? 'Fechar' : 'Abrir'} ordem ${order.plate}`} accessibilityState={{ expanded: open }} onPress={() => setOpenOrderId(open ? null : order.id)} style={({ pressed }) => ({ gap: 14, opacity: pressed ? 0.7 : 1 })}>
          <View style={shared.between}><Text selectable style={shared.muted}>OS #{order.id.slice(-6).toUpperCase()}</Text><StatusBadge status={visibleStatus} /></View>
        ...
        <View style={{ backgroundColor: colors.pale, padding: 14, borderRadius: 10, gap: 5 }}><Text style={shared.label}>Etapa atual</Text><Text style={shared.body}>{visibleStatus}</Text></View>
        ...
        {order.history.length ? order.history.map((event, index) => <Text key={index} style={shared.muted}>{dateLabel(event.occurredAt.toDate())} · {clientStage(event.stage)}{'\n'}{event.note}</Text>) : <Text style={shared.muted}>Veículo recebido pela oficina.</Text>}
